  ...
  customElements.define(
    ...
    class ProductInfo extends HTMLElement {
      ...
      handleOptionValueChange({ data: { event, target, selectedOptionValues } }) {
        ...
        const optimisticVariant = !shouldSwapProduct ? this.getSelectedVariantFromCurrentOptions() : null;

        if (!shouldSwapProduct) {
          if (optimisticVariant) {
            this.updateVariantInputs(optimisticVariant.id);
      ...
      getSelectedVariantFromCurrentOptions() {
        ...
        const selectedOptions = Array.from(this.variantSelectors.querySelectorAll('.product-form__input'))
          .map((optionGroup) => {
            const selectedOption = optionGroup.querySelector('select option:checked');
            const selectedInput = optionGroup.querySelector('input:checked');
            return selectedOption?.value || selectedInput?.value;
          })
          .filter(Boolean);

        if (!selectedOptions.length || selectedOptions.length !== variants[0]?.options?.length) return null;
      ...
      updateSelectedVariantJson(variant) {
      ...
      updateMedia(html, variantFeaturedMediaId) {
        ...
        const applyMediaUpdate = () => {
          sourceGallery?.classList.add('is-changing-media');
          ...
          replaceGalleryList('media-gallery .thumbnail-grid-2x6');
          ...
          requestAnimationFrame(() => {
            sourceGallery?.classList.remove('is-changing-media');
            sourceGallery?.removeAttribute('aria-busy');
            document.dispatchEvent(new CustomEvent('variant:media:updated', { detail: { sectionId: this.dataset.section } }));
          });
        };

        sourceGallery?.removeAttribute('aria-busy');
        sourceGallery?.classList.remove('is-preloading-media');
        applyMediaUpdate();
      }
